...
for i, wt in enumerate(worker_types):
	fig.add_trace(go.Scatter(x=df['Quarter'], y=df[wt].round(1), mode='lines+markers', name=wt, line=dict(color=worker_color_map.get(wt, color_panel[i%len(color_panel)]))))
fig.update_layout(
...
# Calculate QoQ and YoY growth, rounded to 1 decimal
qoq_growth = (df[growth_worker_type].pct_change() * 100).round(1)
yoy_growth = (df[growth_worker_type].pct_change(periods=4) * 100).round(1)
